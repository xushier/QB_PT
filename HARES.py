...
cron: 30 0/3 * * * *
任务名称
new Env('HaresClub 刷流');

变量
export HARES_COOKIE=""
export HARES_RSS_URL=""
export HARES_CONFIG=""
'''
import sys,re,os
...
if not config[site + '_COOKIE']:
    print("请检查 HARES_COOKIE 变量是否设置！")
    sys.exit(1)
if not config[site + '_RSS_URL']:
    print("请检查 HARES_RSS_URL 变量是否设置！")
    sys.exit(1)
if not config[site + '_COOKIE']:
    print("请检查 HARES_CONFIG 变量是否设置！设置举例，5-200-25，即筛选5到200G大小的种子，并且设置上传限速25M")
    sys.exit(1)
...
free_re_rule = r'<\/span><span class=\'layui-pipe\'>|<\/span> <b class=\'(free|twoup|twouphalfdown|twoupfree|thirtypercent)\'>.*<\/b>'
