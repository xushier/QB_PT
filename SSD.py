...
变量
export SPRINGSUNDAY_COOKIE=""
export SPRINGSUNDAY_RSS_URL=""
export SPRINGSUNDAY_CONFIG=""
'''
import sys,re,os
...
if not config[site + '_COOKIE']:
    print("请检查 SPRINGSUNDAY_COOKIE 变量是否设置！")
    sys.exit(1)
if not config[site + '_RSS_URL']:
    print("请检查 SPRINGSUNDAY_RSS_URL 变量是否设置！")
    sys.exit(1)
if not config[site + '_COOKIE']:
    print("请检查 SPRINGSUNDAY_CONFIG 变量是否设置！设置举例，5-200-25，即筛选5到200G大小的种子，并且设置上传限速25M")
    sys.exit(1)
...
free_torrents = Get_Free(cookie, rss_url, run_log).get_free_torrents(temp_log, category, min_size=min_size, max_size=max_size, filter_hr=False)
qb = Client()
