...
变量
export EASTGAME_COOKIE=""
export EASTGAME_RSS_URL=""
export EASTGAME_CONFIG=""
'''
import sys,re,os
...
if not config[site + '_COOKIE']:
    print("请检查 EASTGAME_COOKIE 变量是否设置！")
    sys.exit(1)
if not config[site + '_RSS_URL']:
    print("请检查 EASTGAME_RSS_URL 变量是否设置！")
    sys.exit(1)
if not config[site + '_COOKIE']:
    print("请检查 EASTGAME_CONFIG 变量是否设置！设置举例，5-200-25，即筛选5到200G大小的种子，并且设置上传限速25M")
    sys.exit(1)
...
free_torrents = Get_Free(cookie, rss_url, run_log).get_free_torrents(temp_log, category, min_size=min_size, max_size=max_size, time_format='%a, %d %b %Y %H:%M:%S +0000')
qb = Client()
